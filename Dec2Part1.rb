...
def is_valid?(char, range, password)
...
INPUT.each do |s|
  ...
  char = rule[-1]
  range = rule.delete_suffix(char)
  # puts "#{s} || #{char} | #{range} | #{password} | #{is_valid?(char, range, password)}"
  valid_count += 1 if is_valid?(char, range, password)
